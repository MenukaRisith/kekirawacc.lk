// app/routes/admin.events.new.tsx
import type {
  ActionFunction,
  LoaderFunction,
  MetaFunction,
...
export const action: ActionFunction = async ({ request }) => {
  ...
  try {
    await pool.query(
      `
        INSERT INTO Event
        (title, slug, description, location, startDate, endDate, coverImage, category, status, publishedAt, clubId, createdById)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `,
      [
        title,
...
export default function AdminEventsNewPage() {
  ...
  const defaultLocation = actionData?.fields?.location || "";
  const defaultStartDate = actionData?.fields?.startDate || "";
  const defaultEndDate = actionData?.fields?.endDate || "";
  const defaultCoverImage = actionData?.fields?.coverImage || "";
  const defaultClubId = actionData?.fields?.clubId || "";

  const isClubRep = user.role === "CLUB_REP";

  return (
    <AdminLayout user={user} title="New event">
      <div className="max-w-3xl">
        <h2 className="text-base font-semibold text-gray-900 mb-2">
          Create event
        </h2>
        <p className="text-xs text-gray-500 mb-5">
          Add school events, competitions and programs to the public calendar.
        </p>
